...
logging.info("Содержимое .env файла:")
...
if 'WB_API_TOKEN_FILE' in os.environ:
    ...
    logging.INFO("WB_API_TOKEN_FILE не задан в переменных окружения")
...
#    logging.info(f"Filtered {len(adIds) - len(filtered_adIds)} adIds with consistent 400 errors")
...
def collect_fullstat_data(date, ad):
    ...
    if existing_data:
        logging.info(f"Data for ad {ad} on {date_str} already exists in MongoDB. Skipping.")
        return
    
    # Формируем параметры запроса
    params = [{"id": ad, "dates": [date_str, prev_date]}]
    
#    logging.info(f"Sending request for ad {ad} on {date_str}:")
...
#        logging.info(f"Response status code: {res.status_code}")
        ...
        if res.status_code == 200:
            ...
            if not json_output:
                logging.info(f"Received empty response for ad {ad}")
                ...
                logging.info(f"Data for ad {ad} inserted into MongoDB")
        elif res.status_code == 400:
            logging.info(f"Received 400 error for ad {ad}")
            # Обновляем документ в MongoDB, устанавливая флаг ошибки
            collection.update_one(
                {"advertId": ad, "date": date_str},
...
def get_missing_fullstat_dates(adId, date_range):
    ...
    for date in date_range:
        ...
        if not existing_data or (existing_data.get('error') not in ['empty', '400'] and existing_data.get('data') is not None):
...
#    logging.info(f"Processing {len(fullstat_data)} fullstat records")
...
#    logging.info(f"Processing {len(history_data)} history records")
...
def calculate_additional_metrics(data):
    ...
    for key, item in data.items():
        ...
        item['Share_organic_traffic'] = f"{(((open_card_count - clicks) / open_card_count) if open_card_count and clicks and open_card_count != 0 else 0):.2f}%" 
        
        # Рассчитываем конверсию из корзины в заказ
        item['cartToOrderConversion'] = f"{((orders_count / add_to_cart_count) * 100 if add_to_cart_count and orders_count and add_to_cart_count != 0 else 0):.2f}%" 
        
        # Рассчитываем конверсию из трафика в заказ
        item['CR_traffic_order'] = f"{((orders_count / open_card_count) * 100 if orders_count and open_card_count and open_card_count != 0 else 0):.2f}%" 
        
        # Рассчитываем долю рекламных расходов (DRR)
        item['drr'] = f"{(item.get('sum', 0) / orders_sum_rub if item.get('sum', 0) and orders_sum_rub and orders_sum_rub != 0 else 0):.2f}%"
    ...
    for (nm_id, date), item in final_data.items():
        ...
        if sheet_name not in sheet_data:
            sheet_data[sheet_name] = [
                ["Кампания", "Дата", "Показы", "Клики", "Доля рекламных кликов", "СРС", "CTR рекламы средний", "Затраты",
...
#    logging.info(f"Found {len(nmIds)} unique nmIds for date {date}")
#    logging.info(f"nmIds: {nmIds}")
    return list(nmIds)
...
def get_unique_nmIds_from_fullstat():
    ...
    logging.info(f"Найдено {len(nmIds)} уникальных nmIds")
    return list(nmIds)
...
#        logging.info(f"Extracted and filtered advertIds: {json.dumps(adId, indent=2)}")
#        logging.info(f"Extracted unique advertIds for status 7 or 9, 11, changed within last 30 days: {json.dumps(adId, indent=2)}")
    except Exception as e:
        logging.error(f"Error occurred: {e}")
    
    # Обрабатываем каждую рекламную кампанию
    for ad in adId:
#        logging.info(f"Processing adId: {ad}")
    ...
    for date in date_range:
#        logging.info(f"Сбор данных History для {len(unique_nmIds)} nmIds на дату {date}")
